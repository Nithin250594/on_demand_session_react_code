...
class App extends Component {
  state = {tabListId: tabsList[0].tabId}

  onClickTabUpdate = id => {
    this.setState({tabListId: id})
  }

  updateProjectsList = () => {
    const {tabListId} = this.state
    const filteredList = projectsList.filter(
      eachProject => eachProject.category === tabListId,
    )
  ...
  render() {
    const {tabListId} = this.state
    const filteredProjectsList = this.updateProjectsList()
        ...
        <ul className="tabs-container">
          {tabsList.map(tabDetails => (
            <TabItem
              ...
              isActive={tabDetails.tabId === tabListId}
            />
          ))}
        </ul>
